...
class StoreAgentRequest extends FormRequest
{
    ...
    {
        return [
            'email' => ['required', 'email', 'max:191', Rule::unique('agents', 'email')],
            'password' => ['required', 'string', Password::min(8)->mixedCase()->numbers()],
            'first_name' => ['required', 'string', 'max:100'],
            'last_name' => ['required', 'string', 'max:100'],
            'phone' => [ 'nullable', 'string', 'max:30','regex:/^\+[1-9]\d{6,14}$/', Rule::unique('agents', 'phone'), ],
            'profile_image' => ['nullable', 'image', 'mimes:jpg,jpeg,png,webp', 'max:5120'],
            'gender' => ['nullable', Rule::in(['male', 'female', 'other'])],
            'dob' => ['nullable', 'date', 'before:today'],
            'license_number' => ['nullable', 'string', 'max:100', Rule::unique('agents', 'license_number')],
            'license_expires_at' => ['nullable', 'date', 'after_or_equal:today'],
            'experience_years' => ['nullable', 'integer', 'min:0', 'max:127'],
