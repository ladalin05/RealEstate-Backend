...
@push('scripts')
    ...
    <script>
        ...
        $(document).on('submit', 'form', function(e) {
            ...
            console.log('All selected nodes:', tree.getSelectedNodes().map(function(n) {
                return { key: n.key, title: n.title, selected: n.selected, partsel: n.partsel };
            }));

            var selectedPermissions = [];
            tree.visit(function(node) {
                if (node.isSelected() && /^\d+$/.test(node.key)) {
                    selectedPermissions.push(node.key);
                }
            });

            console.log('Filtered permissions to send:', selectedPermissions);
