    ...
    <form action="{{ $action }}" method="POST" id="district-form">
        ...
        <div class="mb-3">
            ...
            <select name="country_id" id="country" class="form-select @error('country_id') is-invalid @enderror" required>
                ...
                @if(isset($form->city_id))
                    <option value="" selected>{{ $form?->city?->country?->name }}</option>
                @endif
            </select>
            @error('country_id')
        ...
        <div class="mb-3">
            ...
            <select name="city_id" id="city" class="form-select @error('city_id') is-invalid @enderror" required>
                ...
                @if(isset($form->city_id))
                    <option value="{{ $form?->city_id }}" selected>{{ $form?->city?->name }}</option>
                @endif
            </select>
            @error('city_id')
                <div class="text-danger small mt-1">{{ $message }}</div>
        ...
        <div class="mb-3">
            <label for="name" class="form-label fw-bold text-dark small text-uppercase">District Name</label>
